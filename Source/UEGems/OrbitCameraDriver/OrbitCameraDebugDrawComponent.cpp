...
static void DrawEllipsoidPolygon(FMeshElementCollector& Collector, const FMaterialRenderProxy* MaterialRenderProxy, int32 NumSides, int32 NumRings, float StartAngleDeg, float EndAngleDeg, const FVector& EllipsoidRadius, const FVector& Center, const FRotator& Orientation, int32 ViewIndex)
{
	...
	FDynamicMeshBuilder MeshBuilder(Collector.GetFeatureLevel());
	{
		...
		FDynamicMeshVertex* Verts = static_cast<FDynamicMeshVertex*>(FMemory::Malloc(NumVerts * sizeof(FDynamicMeshVertex)));

		// Calculate verts for one arc
		FDynamicMeshVertex* ArcVerts = static_cast<FDynamicMeshVertex*>(FMemory::Malloc((NumRings + 1) * sizeof(FDynamicMeshVertex)));

		for (int32 i = 0; i<NumRings + 1; i++)
		{
			FDynamicMeshVertex* ArcVert = &ArcVerts[i];
			
			float Angle = FMath::DegreesToRadians(StartAngleDeg) + ((float)i / NumRings) * (FMath::DegreesToRadians(EndAngleDeg - StartAngleDeg));
		...
		for (int32 s = 0; s < NumSides + 1; s++)
		{
			...
			float XTexCoord = static_cast<float>(s) / NumSides;
...
FDebugRenderSceneProxy* UOrbitCameraDebugDrawComponent::CreateDebugSceneProxy()
{
	...
	public:
		...
		virtual void GetDynamicMeshElementsForView(const FSceneView* View, const int32 ViewIndex, const FSceneViewFamily& ViewFamily, const uint32 VisibilityMap, FMeshElementCollector& Collector, FMaterialCache& DefaultMaterialCache, FMaterialCache& SolidMeshMaterialCache) const override
		{
			...
			DrawEllipsoidPolygon(Collector, SpherePolygonColor, 20, 7, VerticalAngleRangeDeg.X, VerticalAngleRangeDeg.Y, EllipsoidRadius, Location, Orientation, ViewIndex);
		}
		
		FVector EllipsoidRadius;
		
		FVector2D VerticalAngleRangeDeg;
		FVector Location;
	...
	DebugProxy->VerticalAngleRangeDeg = OrbitCameraActor->GetVerticalRotationLimitDegrees();
